...
(:~
 : Interpretes siglum provided and returns a map with references to specific tome, chapters and verses
...
) as map:map
{
    try {
        ...
        let $_PUT_CHAPTERS := if (fn:empty(map:keys($chapters))) then () else map:put($interpretation, 'chapters', $chapters)
